...
import { defaultMapStyles, defaultMapLibreStyles } from "./types";
...
export const Map = ({
  ...
  const createMapInstance = (container) => {
  ...
  const isStandardStyle = (styleUrl) => {
    return styleUrl.includes("mapbox://styles/mapbox/standard");
  }

  const updateStandardLightPreset = (mapInstance) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      return () => {
        delete (container).getBoundingClientRect
